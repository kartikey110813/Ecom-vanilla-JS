...
if ('serviceWorker' in navigator) {
  ...
  function adCard(data, key){
    ...
    if(firebase.auth().currentUser === null){
      ...
      <div class="cardstyling col-lg-4 col-sm-6 portfolio-item">
        ...
        <h5>Rs. ${data.price}</h5>
        <button type="button" class="btnQty" onclick="signInFirst()">Quantity</button>
        <button type="button" class="btnCart" onclick="signInFirst()">Add To Cart</button>
          </div>
        </div>
      </div>
    `
   ...
   return`
    <div class="cardstyling col-lg-4 col-sm-6 portfolio-item">
      <div class="card h-100">
        ...
        <p class="validate card-text">${data.description}</p>
        <h5>Rs. ${data.price}</h5>
        <button type="button" class="btnQty" data-toggle="modal"  onclick="SelectQty()">Quantity</button>
        <button type="button" class="btnCart" onclick="addToFavourites(this)">Add To cart</button>
      </div>
      </div>
    </div>
  `}
    ...
    const adsRef = database.ref("Admin products");
  ...
  function addToFavourites(button){
    ...
    // + `url :${url}` + `\n`
    // );
    database.ref('cart/' + firebase.auth().currentUser.uid).push().set({
      category : category,
      description : description,
      displayName : displayName,
      price : price,
  ...
  function SelectQty() {
     quantity =  prompt("Please mention the quantity here");
  }

  function quantityFirst() {
  ...
  //   database.ref('Admin products/' + adKey +`/messages`).push().set({
  ...
  //   var messagesRef = database.ref('Admin products/' + key +`/messages`);
